...
c_p = 1005;                           % Specific heat capacity of air (J/kg·°C)
C_r = m_air * c_p;                    % Thermal capacitance of air in the box (J/°C)
...
ss_plant = sys( ...    
    struct( ...  % params
        ...
        "Ch",                   3, ...
        "c_p",                c_p, ...
        "k_walls",              5.3, ...                     % heat loss from room to environment
        "kA_h_loss",             0.1, ...                     % heat loss from heater to environment
        "P_l",                  20, ...
        "P_h",                  112.5, ...
        "tau_s",                2, ...
        "tau_v",                2, ...
...
T_ho = T_hi + (S.c_p * S.rho * s * S.A_pipe * (S.T_h - T_hi) ...
        / (m_dot * S.c_p));

dT = ( ...
    S.k_walls * S.A_room * (S.T_ext - S.T) ...
    + S.P_l * S.u_l ...
    + m_dot * S.c_p * (T_ho - S.T) ...
) / S.Cr;

dT_h = ( ...
    S.kA_h_loss * (S.T_ext - S.T_h) ...
    - S.c_p * S.rho * s * S.A_pipe * (S.T_h - T_hi) ...
    + S.P_h * S.u_h ...
) / S.Ch;
