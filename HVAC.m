...
variables = { 'T', 'T_vo', 'T_h', 'T_ho', 'T_ext', 'light', ...
    'm_dot_i', 'm_dot_e', 'u_h', 'u_bulb', ...
    'C_room', 'k_room', 'A_room', ...
    'C_vo', 'k_pipe', 'A_pipe', ...
    'C_h', 'k_h', 'A_h', 'K_h', ...
    'C_ho', 'alpha' ,...
    'C_air', 'P_bulb', 'eff_bulb', 'P_h'};
...
T_vo_0 = 20;             % Nominal temperature of temp sensor after the valve
T_ho_0 = 20;             % Nominal temperature of temp sensor after the heater
...
Q_T_loss = k_room * A_room * (T - T_ext);             % heat loss through the walls
Q_T_ho = m_dot * C_air * (T_ho - T);                  % heat change from ventillation
Q_T_bulb = P_bulb * u_bulb * (1 - eff_bulb);          % Heat generated by bulb

dTdt = (-Q_T_loss + Q_T_ho + Q_T_bulb) / C_room;

% Equations for heat change after the valve
Q_vo_loss = k_pipe * A_pipe * (T_vo - T_ext);
Q_vo_T = m_dot_i * C_air * (T - T_vo);
Q_vo_T_ext = m_dot_e * C_air * (T_vo - T_ext);

dTvodt = (-Q_vo_loss + Q_vo_T + Q_vo_T_ext) / C_vo;

% Heating element temperature change (dT_h/dt)
Q_h_heater = u_h * P_h;
Q_h_loss = k_h * A_h * (T_h - T_ext);
Q_h_vo = m_dot * K_h * (T_h - T_vo);
dThdt = (Q_h_heater - Q_h_loss - Q_h_vo) / C_h;


% Equations for heat change after the valve
Q_ho_loss = k_pipe * A_pipe * (T_ho - T_ext);
Q_ho_vo = m_dot * C_air * (T_ho - T_vo);
Q_ho_Th = m_dot * K_h * (T_h - T_ho);

dThodt = (-Q_ho_loss + Q_ho_vo + Q_ho_Th) / C_ho;
...
ss_X = [T; T_vo; T_h; T_ho; T_ext];                                         % states
ss_Y = [T; T_vo; T_ho; light];                                              % outputs
ss_f = [dTdt; dTvodt; dThdt; dThodt; dTextdt];                              % would be if already linear
pretty(ss_f);

ss_f = taylor([dTdt; dTvodt; dThdt; dThodt; dTextdt], [ ...
    T, T_vo, T_h, T_ho, T_ext, m_dot_i, m_dot_e], ...                       % which variables are used to linearize
    'ExpansionPoint', ...
    [T_0, T_vo_0, T_h_0, T_ho_0, T_ext_0, T_ho_0, T_ext_0], ...             % around what poins are they linearized
    'Order', 2);
pretty(ss_f);
% Define second state space for plant simulation
plant_U = [T_ext; m_dot_i; m_dot_e; u_bulb; u_h];                           % inputs / controlled variables
plant_X = [T; T_vo; T_h; T_ho; T_ext];                                      % states
plant_Y = [T; T_vo; T_ho; light];                                           % outputs
plant_f = taylor([dTdt; dTvodt; dThdt; dThodt; dTextdt], [ ...
    T, T_vo, T_h, T_ho, T_ext, m_dot_i, m_dot_e], ...                       % which variables are used to linearize
    'ExpansionPoint', ...
    [T_0, T_vo_0, T_h_0, T_ho_0, T_ext_0, m_dot_i_0, m_dot_e_0], ...        % around what poins are they linearized
    'Order', 2);
...
A_room = 2*a*b+2*a*c+2*b*c;     % Surface area of walls (m²)
C_air = 1005;                   % Specific heat capacity of air (J/kg·°C)
C_room = m_air * C_air;         % Thermal capacitance of air in the box (J/°C)

k_room = 1;                     % Heat transfer coefficient of walls (W/m²°C)
k_pipe = 0.2;                   % Heat transfer coefficient of pipes (W/m²°C)
A_pipe = 0.3;                   % Surface area of pipes (m²)
C_vo = 0.2;
C_ho = 0.2;
k_h = 0.2;
A_h = 0.030;
C_h = 50;                       % Thermal capacitance of heating element (J/°C)
K_h = 10;                       % Heat transfer coefficient air in heater (W/°C)
...
disp('Numeric A matrix:'); mPretty(A);
disp('Numeric B matrix:'); mPretty(B);
disp('Numeric C matrix:'); mPretty(C);
disp('Numeric D matrix:'); mPretty(D); 
...
% X = [T; T_vo; T_h; T_ho; T_ext]  % penatize bad performance
Q = diag([1 1 1 1 1]);
